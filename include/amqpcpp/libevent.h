...
private:
    ...
    std::map<TcpConnection*, event *>  _timer_events;
    ...
    {
        auto titer = _timer_events.find(connection);
        if (titer != _timer_events.end()) {
            evtimer_del(titer->second);
            event_free(titer->second);
        }
    }
    
    void setTimer(AMQP::TcpConnection * connection, uint16_t interval)
    {
        struct timeval time;
        time.tv_sec = interval/2;
        time.tv_usec = time.tv_sec != 0 ? 0 : 500000;

        event* ev = event_new(_evbase, -1, EV_PERSIST, timerCallback, connection);
        _timer_events[connection] = ev;
        evtimer_add(ev, &time);
    }
