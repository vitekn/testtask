...
class ProxyTest(unittest.TestCase):
    def test_1one_client_one_server(self):
        ...
        for param in params:
            ...
            c1 = Client(param[0], proxy_addr, 12345)
            c1.start()
    ...
    def test_2two_clients_one_server(self):
        s1 = Server(8888)
        s1.start()

        time.sleep(1)

        c1 = Client(1, proxy_addr, 12345)
        c2 = Client(2, proxy_addr, 12345)
        c1.start()
        i = 0
    ...
    def test_3many_clients_many_servers(self):
        servers = [Server(8888), Server(8889), Server(8890)]
        clients = [Client(1, proxy_addr, 12345), Client(3, proxy_addr, 12345), Client(4, proxy_addr, 12345)]
    ...
    def test_4rmq_close_clients(self):
        servers = [Server(8888), Server(8889), Server(8890)]
        clients = [
            Client(1, proxy_addr, 12345),
            Client(3, proxy_addr, 12345),
            Client(1, proxy_addr, 12345),
            Client(4, proxy_addr, 12345)]
        for server in servers:
            server.start()
        time.sleep(1)
        ...
        cr = pika.PlainCredentials(rmq_user, rmq_pass);
        connection = pika.BlockingConnection(pika.ConnectionParameters(rmq_addr,credentials=cr))
...
if __name__ == '__main__':
    ...
    proxy_addr = os.environ.get('TEST_PROXYADDR', "localhost")
    rmq_addr = os.environ.get('TEST_RMQADDR', "localhost")
    rmq_user = os.environ.get('TEST_RMQUSER', "guest")
    rmq_pass = os.environ.get('TEST_RMQPASS', "guest")
